...
import '../../assets/styles/Breeds.css';

import { useAppSelector, useAppDispatch } from '../../app/hooks';
import {
  setBreeds as setBreedsList,
  selectBreeds,
  getBreedsAsync,
  getBreedImagesAsync,
} from './breedsSlice';
...
export const Breeds = () => {
  const breedsList = useAppSelector(selectBreeds);
  ...
  const searchName = async (searchResult: string) => {
    if (searchResult.length >= 3) {
      await dispatch(getBreedsAsync(searchResult));
      await dispatch(getBreedImagesAsync());
    }
  }
  ...
  const sorting = (sortResult: string) => {
    ...
    dispatch(setBreedsList(sortData));
  };
